...
def make_pipeline():
    ...
    v_gross_profit = Fundamentals.gross_profit.latest
    v_investing_cash_flow = Fundamentals.investing_cash_flow.latest
    v_long_term_investments = Fundamentals.long_term_investments.latest
    ...
    gross_profit_winsorized = v_gross_profit.winsorize(min_percentile=0.05,max_percentile=0.95)
    investing_cash_flow_winsorized = v_investing_cash_flow.winsorize(min_percentile=0.05,max_percentile=0.95)
    long_term_investments_winsorized = v_long_term_investments.winsorize(min_percentile=0.05,max_percentile=0.95)
    ...
    sum_combined_factor = (
        ...
        gross_profit_winsorized+
        investing_cash_flow_winsorized+
       long_term_investments_winsorized
    )
    
    wgt_sum_combined_factor = (
        value_winsorized*(value_winsorized/sum_combined_factor)+ 
        quality_winsorized*(quality_winsorized/sum_combined_factor) + 
        sentiment_score_winsorized*(sentiment_score_winsorized/sum_combined_factor)+
        gross_profit_winsorized*(gross_profit_winsorized/sum_combined_factor)+
        investing_cash_flow_winsorized*(investing_cash_flow_winsorized/sum_combined_factor)+
        long_term_investments_winsorized*(long_term_investments_winsorized/sum_combined_factor)
    )
    ...
    # day.
    longs = wgt_sum_combined_factor.top(TOTAL_POSITIONS//2, mask=universe)
    shorts = wgt_sum_combined_factor.bottom(TOTAL_POSITIONS//2, mask=universe)
    ...
    pipe = Pipeline(
        columns={
            'longs': longs,
            'shorts': shorts,
            'combined_factor': wgt_sum_combined_factor
        },
